...
impl<T> Binning<T>
    where T: Float
    // Add<Output=T> + Div<Output=T> + Mul<Output=T> + Sub<Output=T> + From<u32> + From<f32>
{
    fn bin(&self, n: T) -> Option<u16> {
        match cast((n - self.min) * self.scale) {
            Some(c) => if c >= self.num { None } else { Some(c) },
            _ => None
        }
    }
    ...
    fn iter(&self) -> impl Iterator<Item = T> + '_ {
        (0..self.num).map(move |n|
                          self.min + (cast::<u16, T>(n).unwrap() +
                                      cast::<f32, T>(0.5).unwrap()) /
                          self.scale)
    }
}
    ...
    where T: Float
{
    ...
    pub fn new(xmin: T, xmax: T, xnum: u16,
               ymin: T, ymax: T, ynum: u16,
               bins: Vec<&'a mut [u32]>) -> Histogram<'a, T>
    {
        let xaxis = Binning { scale: T::from(xnum).unwrap() / (xmax - xmin),
                              min: xmin, num: xnum };
        let yaxis = Binning { scale: T::from(ynum).unwrap() / (ymax - ymin),
                              min: ymin, num: ynum };
    ...
    where T: Float
{
    ...
    ComplexSequence { z: Complex { re: start, im: start }, c: c, r: radius }
}
